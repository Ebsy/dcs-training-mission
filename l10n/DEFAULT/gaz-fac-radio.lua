...
fac.FAC_smokeColour_BLUE = 2 -- BLUE side smoke colour -- Green = 0 , Red = 1, White = 2, Orange = 3, Blue = 4

fac.FAC_FACStatusF10 = true -- enables F10 FAC Status menu

fac.FAC_location = true -- shows location of target in FAC message

fac.FAClock = "vehicle" -- "vehicle" OR "troop" OR "all" forces FAC to only lock vehicles or troops or all ground units
        ...
        for _, _facUnitName in ipairs(fac.facPilotNames) do
            ...
            if _facUnit ~= nil then
                ...
                if fac.facUnits[_facUnitName] == nil and fac.facOnStation[_facUnitName] == true then
                    ...
                    fac.facAutoLase(_facUnitName) --(_facUnitName, _laserCode, _smoke, lock, _colour)
                end
            end
        end
    end)
...
function fac.facAutoLase(_facUnitName, _laserCode, _smoke, lock, _colour)
    ...
    if _facUnit:isActive() == false then
        ...
        timer.scheduleFunction(fac.timerFacAutoLase, { _facUnitName, _laserCode, _smoke, lock, _colour }, timer.getTime() + 30)
    ...
    if _enemyUnit ~= nil then
        ...
        --
        timer.scheduleFunction(fac.timerFacAutoLase, { _facUnitName, _laserCode, _smoke, lock, _colour }, timer.getTime() + 1)
...
--[[
        ...
        timer.scheduleFunction(fac.timerFacAutoLase, { _facUnitName, _laserCode, _smoke, lock, _colour }, timer.getTime() + 5)
    end
end
...
function fac.findFacNearestVisibleEnemy(_facUnit, _targetType,_distance)
        ...
        if _enemyGroups[_i] ~= nil then
            ...
            if #_units > 0 then
                for _y = 1, #_units do
                    ...
                    if _tempDist < _maxDistance then
                ...
                for _x = 1, #_units do
                    ...
                    if _units[_x]:isActive() == true and _targeted == false and _allowedTarget == true then
						...
						if _tempDist < _maxDistance then
                            ...
                            -- calc visible
							if land.isVisible(_offsetEnemyPos, _offsetFacPos) then
								table.insert(_RadioSelectUnits, _units[_x])
							end
						end
                    end
                end
    ...
    if _RadioSelectUnits == nil then
        return nil
    end	
	
return _RadioSelectUnits
end
            ...
            if _unit ~= nil then
                ...
                if _groupId then
                    ...
                    if fac.facAddedTo[tostring(_groupId)] == nil then
						...
						fac.RadioRootPath = missionCommands.addSubMenuForGroup(_groupId, "FAC")
                  --      missionCommands.addCommandForGroup(_groupId, "Go On-Station",  fac.RadioRootPath, fac.setFacOnStation, { _facUnitName, true})
                        missionCommands.addCommandForGroup(_groupId, "Go Off-Station", fac.RadioRootPath, fac.setFacOnStation, { _facUnitName, nil})
												
                        -- add each possible laser code as a menu option
                        for _, _laserCode in pairs(fac.FAC_laser_codes) do
                            missionCommands.addCommandForGroup(_groupId, string.format("Laser code: %s", _laserCode), fac.RadioRootPath, fac.setFacLaserCode, { _facUnitName, _laserCode})
                        end

                        fac.facAddedTo[tostring(_groupId)] = true
                    end					
			
					-- NEW: Select targets from radio command

					if _NewTargListPath ~= nil then
						missionCommands.removeItemForGroup(_groupId,_NewTargListPath)
					end
					_NewTargListPath = missionCommands.addCommandForGroup(_groupId, "New Target List", fac.RadioRootPath,fac.newRadioList,{_unit,_groupId})		
...
-- NEW: Create a new list of targets
function fac.newRadioList(_newRadioListArgs)

local _GroupID = _newRadioListArgs[2]
	if _SelTargetPath ~= nil then
		missionCommands.removeItemForGroup(_GroupID,_SelTargetPath)
	end
	_SelTargetPath = missionCommands.addSubMenuForGroup(_GroupID, "Select Target", fac.RadioRootPath)
local _newListFacUnit = _newRadioListArgs[1]
local _facUnitName = _newListFacUnit:getName()
if lock == nil then
	lock = fac.FAClock
end
local _foundunits = fac.findFacNearestVisibleEnemy(_newListFacUnit, lock)
	if #_foundunits > 0 then
		for _r=1, #_foundunits do
			if _foundunits[_r] ~= nil then
				local _tempRadioUnit=_foundunits[_r]
				_menuType = _tempRadioUnit:getTypeName()
				missionCommands.addCommandForGroup(_GroupID, _r .. _menuType, _SelTargetPath, fac.selectRadioUnit,{_foundunits[_r],_facUnitName})
			end
		end
	end
end
...
-- NEW: Make the radio selected unit globally available for _enemyUnit and cycle on station
function fac.selectRadioUnit(_selRadioUnitArgs)
	RadioSelectedUnit = _selRadioUnitArgs[1]
	local _tempFacUnitName = _selRadioUnitArgs[2]
	
	fac.setFacOnStation({_tempFacUnitName, nil})
	 timer.scheduleFunction(fac.setFacOnStation, {_tempFacUnitName, true}, timer.getTime() + 3)
end
